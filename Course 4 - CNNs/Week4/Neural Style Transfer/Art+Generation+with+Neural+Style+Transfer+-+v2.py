...
def compute_content_cost(a_C, a_G):
    ...
    a_C_T = tf.transpose(a_C)
 #   print("a_C_T.shape=", a_C_T.shape)
    a_C_unrolled = tf.reshape(tf.transpose(a_C) ,[n_C, n_H * n_W])
    a_G_unrolled = tf.reshape(tf.transpose(a_G) ,[n_C, n_H * n_W])
    

    # compute the cost with tensorflow (≈1 line)
  #  print ("Sqrt_diff.shape= ", tf.square(tf.subtract(a_C_unrolled, a_G_unrolled)).shape)
    J_content = tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled, a_G_unrolled))) / (4 * n_H *  n_W * n_C)
   # print ("J_content.shape=", J_content.shape)
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
   ...
   # print ("a_S.shape=", tf.reshape(tf.transpose(a_S), [n_C, n_H * n_W]).shape )
