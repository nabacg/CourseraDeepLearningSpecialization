...
def rnn_cell_forward(xt, a_prev, parameters):
    ...
    a_next = np.tanh(Waa @ a_prev + Wax @ xt + ba) # ( (n_a, n_a) *(n_a, 1)  (n_a, n_x) * (n_x, m)  + (n_a, 1))
    # compute output of the current cell using the formula given above
    yt_pred = softmax(Wya @ a_next + by) # (n_a, n_a) * (n_a, 1) + (n_a, 1)
    ### END CODE HERE ###
...
concat = np.zeros((3,2))   # np.concatenate(a_prev, xt, axis=0)
concat[: 2, :] = a
concat[2 :, :] = b
concat
...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    concat = np.zeros((n_x+n_a, m))   # np.concatenate(a_prev, xt, axis=0)
    concat[: n_a, :] = a_prev
    concat[n_a :, :] = xt
...
def rnn_cell_backward(da_next, cache):
    ...
    dtanh = 1 - np.square( np.tanh(dx))

    # compute the gradient of the loss with respect to Wax (≈2 lines)
    dxt = None
    dWax = (1 - np.square(np.tanh())) @ dxt
